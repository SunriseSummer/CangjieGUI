...
FIXTURE_OUTPUT = DEV_TARGET_ROOT / "fixtures" / "desktop_lifecycle"
REPORT = DEV_TARGET_ROOT / "test-package-results" / "desktop-lifecycle.json"
PASS_MARKER = "@@DESKTOP_LIFECYCLE|passed"
DAMAGE_INCREMENTAL = FIXTURE_OUTPUT / "retained-damage.bmp"
...
def _collect_damage_snapshots():
    """Move runtime snapshots out of the source fixture before comparison."""
    FIXTURE_OUTPUT.mkdir(parents=True, exist_ok=True)
    for source, destination in (
            (DAMAGE_SOURCE_INCREMENTAL, DAMAGE_INCREMENTAL),
            (DAMAGE_SOURCE_FULL, DAMAGE_FULL)):
        if source.is_file():
