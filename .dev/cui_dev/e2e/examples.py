...
real-render snapshots with BMP baselines through the shared snapshot module.

Typical use:
    python .dev/cli.py test examples                         # test every example
    python .dev/cli.py test examples --action build --jobs 4
    python .dev/cli.py test examples contacts planner --snapshot
...
RESULTS = DEV_TARGET_ROOT / "examples"
