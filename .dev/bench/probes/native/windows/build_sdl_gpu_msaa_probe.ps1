...
$repo = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))))
$target = Join-Path $repo 'target\bench\probes\native'
$archive = Join-Path $target 'SDL3-3.4.12.zip'
$sourceRoot = Join-Path $target 'SDL3-3.4.12-source'
$headers = Join-Path $sourceRoot 'SDL3-3.4.12\include'
$source = Join-Path $repo '.dev\bench\probes\native\windows\sdl_gpu_msaa_probe.cpp'
$executable = Join-Path $target 'sdl_gpu_msaa_probe.exe'
$runtimeCopy = Join-Path $target 'SDL3.dll'
$report = Join-Path $repo 'target\bench\results\sdl-gpu-msaa-probe.json'
...
if (-not (Test-Path -LiteralPath (Join-Path $headers 'SDL3\SDL.h') -PathType Leaf)) {
    throw "SDL 3.4.12 headers were not found after extraction: $headers"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "SDL GPU MSAA probe compilation failed with exit code $LASTEXITCODE"
}
Copy-Item -LiteralPath $SdlRuntime -Destination $runtimeCopy -Force

Write-Output "Built $executable against verified SDL 3.4.12 headers."
if ($Run) {
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $report) | Out-Null
    & $executable --width $Width --height $Height --warmup $Warmup --frames $Frames --repeats $Repeats --output $report
    if ($LASTEXITCODE -ne 0) {
        throw "SDL GPU MSAA probe failed with exit code $LASTEXITCODE"
    }
