...
| | 保留式组件对象模型 |
|---|---|
| **消失** | 显式的留存调用；在只支持 keyed remember 的框架中还包括 `rememberState("key")` 的命名仪式 |
...
一条务实的折中路线是：先在稳定身份作用域上提供受形状守卫的位置留存——例如 CUI 的 `remember { ... }` / `rememberState { ... }`，固定声明中的普通对象与可写状态都不再命名，条件/循环/重排才使用显式业务键；再把“组件对象字段”作为这种位置槽的语法糖。如此可逐步获得接近“状态即字段”的体验，却不引入可变组件实例生命周期。位置槽必须验证形状并事务回滚，否则同类型条件插入会静默串位；稳定工厂还应把内部派生图标记为长期反应节点，否则对象虽被留存，依赖图仍按短命对象处理。这些正确性与生命周期边界比是否采用对象语法更重要。
