...
行是 `item` 构建器产出的普通组件，可携带自己的内容与控件：落在手柄条（每行左缘 30 逻辑像素）之外的事件自顶向下转发给行内容，勾选、点按都与拖动互不干扰；显式传入的 `Frame` 会转发给全部行；桌面宿主正常通过独立订阅表推进帧回调。行标识跟随 `key`——给出稳定键后，勾选态等行内局部状态在重排后仍跟随原行。
