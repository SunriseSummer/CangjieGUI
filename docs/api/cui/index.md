...
| [`structuralEqualityPolicy`](core/functions.md#structuralequalitypolicy) | 返回以 `==` 抑制 State 相等写入的策略。 |
...
| [`imageCacheStats`](media/functions.md#imagecachestats) | 返回调用 UI 线程的图像缓存诊断快照。 |
...
| `WindowSpec` | 创建窗口时的一次性选项：标题、逻辑尺寸、DPI 与缩放行为、垂直同步和渲染器的超采样策略；默认 0 按物理密度 Auto，正数为精确倍数。 |
