...
外部滚动状态为 NaN 时恢复到顶部，正／负无穷分别限制到底部／顶部；布局前完成校正，避免无效坐标进入绘制。滚动偏移是距内容顶部的逻辑像素，始终被限制在 0 与最大偏移（内容高 − 视口高，不小于 0）之间；滚轮每格步进 72 逻辑像素。滚轮先交给指针下的子控件；子控件无法继续滚动时本层接续。内容装得下、已到该方向边界、零增量或非有限增量时不消费事件。滚动条只在内容溢出时出现：内容让出条的槽宽（10 逻辑像素）以免被遮，滑块可拖拽（记录抓取点、不跳变），主键点击滑块之外的轨道按 0.9 视口高分页。

偏移与滑块拖动状态标识默认由声明顺序自动派生；树的结构会变化（条件分支、动态列表）时给显式 `key!`，或用 [`scrollState`](#scrollstate) 换成外部持有的状态以便程序化滚动（如"发送后滚到底部"）。绘制在滚动轴上按逻辑边界裁剪，交叉轴留出阴影余量，视口边缘卡片的软阴影不会被竖直切掉。

SDL 的原生裁剪矩形使用整数逻辑坐标；小数边界向外取整覆盖完整内容，每边最多扩张不到一个逻辑像素。命中与语义边界仍按原始逻辑矩形计算，空交集保持为空。
...
| [`draw(ctx: UiContext)`](#draw) | 在滚动轴上裁剪绘制内容，内容溢出时再绘制滚动条。 |
...
在滚动轴上裁剪绘制内容，内容溢出时再绘制滚动条。交叉轴留出阴影余量，视口边缘卡片的圆角软阴影不被竖直切掉；拖拽中的滑块以高亮绘制。
