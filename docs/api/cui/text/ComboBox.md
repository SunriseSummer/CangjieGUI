...
- **展开浏览**：点击箭头或聚焦后按 Down，先显示全部选项，并高亮／滚动到当前值；已有值不自动作为过滤条件。展开时选中编辑框里的现有文字，直接输入即可替换为查询。
- **过滤**：编辑后按 ASCII 大小写不敏感的子串匹配过滤，空输入显示全部选项；重新输入会重置高亮与滚动位置。没有匹配时弹层显示占位行 `—`，点击它只是关闭列表。
...
| 成员 | 说明 |
|---|---|
| [`measure(...)`](#measure) | [`Widget`](../core/Widget.md) 协议实现：占满可用宽度，高度由内置 TextField 的行高与上下内边距决定，至少 38 逻辑像素。 |
...
[`Widget`](../core/Widget.md) 协议实现：占满可用宽度，高度由内置 TextField 的行高与上下内边距决定，至少 38 逻辑像素。
