...
| [`cui.media`](cui/media/index.md) | 图像与自绘：文件图像视图 ImageView（线程/渲染器隔离的有界纹理缓存）与自由绘制表面 CanvasWidget。 |
| [`cui.desktop`](cui/desktop/index.md) | 桌面应用对象：DesktopApp 拥有窗口与按需渲染循环，驱动稳定构建/布局、事件事务、增量或全量绘制。 |
| [`cui.testing`](cui/testing/index.md) | 确定性帧宿主与构建、布局、事件、绘制、damage、文本探针。 |
