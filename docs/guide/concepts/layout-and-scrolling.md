...
表单页面可以让外层 `VStack` 同时包含一个 `.flex()` 的 `ScrollView` 和底部保存按钮。这样只有会增长的字段区滚动，主要操作始终可达。基础状态与表单组合见[设置表单教程](../tutorials/settings-form.md)。
...
`ScrollView` 和 Lazy 容器先把滚轮交给指针下的子控件，再处理自身滚动。`ListView`、`Table`、`TreeView`、`TextArea` 在偏移无法继续改变时返回未消费，外层视口可以接续。下拉弹层保留自己的滚轮隔离策略，避免滚动背景。

滚动条仅响应鼠标主键；最小滑块长度受轨道实际长度限制，缩小到极小视口也不会越界。小数布局的绘制裁剪采用保守整数边界，避免底部或右侧内容被截去；逻辑命中与无障碍可见范围不随之扩张。卡片边框画在布局框内，应用无需额外加空白来补偿裁剪错误；阴影等有意超出布局的效果仍需设计相应留白。
