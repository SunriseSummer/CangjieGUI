...
| `cui.core` | Widget 协议、上下文、状态、主题、布局容器（含惰性列表）、基础控件与浮层机制 |
| `cui.controls` | 复合控件：选择、导航、数值、菜单与对话框浮层、富文本展示，以及日期时间值类型 |
...
`DesktopApp.run` 的帧循环在需要重绘的帧执行视图构建闭包，随后依次完成布局、事件分发、帧事件和
绘制（何时重绘见下文“帧调度与脏帧跳过”）。共享状态保存在模型或提升后的 `State<T>` 中；局部状态由
`StateStore` 按 `Keyed` scope 与显式 key 保存。成功构建后没有再次访问的局部状态会被清理，因此状态
生命周期与声明式子树的挂载生命周期一致。
...
帧循环按需重绘而非无条件每帧重建：仅当本帧可能与上帧不同时才构建、布局、分发事件并绘制——
首帧、有待处理输入、上次渲染以来有状态写入、视口尺寸变化，或上帧请求了连续帧（闪烁光标、
倒计时中的提示、逐帧 `FrameHandler`、运动中的弹簧动画）。其余情况只按帧节奏轮询输入、保留上次
呈现的画面，空闲窗口因此不付出构建/布局/绘制成本。

判定依据两个信号。其一，`State` 每次赋值自增一个进程级写代际计数（`currentStateGeneration`），
循环在帧顶部快照该值，故本帧布局/分发/绘制期间发生的写入必定在下一帧被反映，不丢更新；
后台线程的写入同样计数（增量非原子，但丢失的自增只会偶尔多绘一帧，绝不漏掉真实变化）。其二，
连续行为经 `UiContext.requestFrame()` 请求下一帧，循环用 `takeFrameRequest()` 读清。由此任何靠帧
时钟推进的时间动画都必须主动续帧，否则在空闲跳帧下会冻结——这是脏帧跳过对全框架的一条硬约束，
弹簧动画统一走 `Spring.animate(ctx, target:)` 自动满足（见“动画”）。
...
动画不需要独立的定时器或失效追踪，渲染循环本身就是动画时钟：`Spring` 原语（弹簧-阻尼数值）由控件
在 `draw` 里按帧间隔 `tick` 推进、读取 `value`。因帧循环空闲跳帧（见“帧调度与脏帧跳过”），运动中的
弹簧必须主动续帧，否则会在过渡中途被跳过而冻结——控件统一用 `animate(ctx, target:)`，它在尚未停住时
调用 `ctx.requestFrame()` 让下一帧继续绘制，停住即不再请求，故空闲弹簧零开销、静态窗口不出帧。积分用
半隐式欧拉并钳制步长，使一次帧卡顿（长间隔）不会注入巨大冲量导致发散；到达目标后精确停住。
动画状态必须跨帧保留（`localState`），而 `localState` 仅在构建期可用、`draw` 阶段不可用——因此控件
在构造函数里取回被保留的 `Spring` 存为字段，绘制时复用（`animate(ctx, target:)` 一步完成
“设目标+推进+读值+续帧”）。弹簧初始化在控件当前值即“已停住”，只有状态其后改变才动画，故静态首帧
...
`animate(target:, deltaMs:)`（无 ctx 版，不续帧）只供测试与自驱时间的调用方，控件绘制勿用。
...
CUI 当前仍采用确定性的完整重建（每次重绘构建整棵树，仅空闲帧整体跳过，不做细粒度增量），尚未
声称实现细粒度重组或自调整计算。未来若加入依赖追踪，
