...
macOS 不使用仓库内的 Windows x86_64 DLL。先安装与当前架构匹配的 SDL3 和 SDL3_ttf：
...
脚本通过 `brew --prefix` 定位动态库，并复制为：

```text
sdl/.sdl3/libSDL3.dylib
sdl/.sdl3/libSDL3_ttf.dylib
```

这些文件沿用现有 `[ffi.c]` 搜索目录，但被 `.gitignore` 排除。脚本会在覆盖前恢复目标文件的
用户写权限，因此 Homebrew 更新后可以重复执行。Windows DLL 不会被修改。
...
Homebrew bottle 的最低系统版本由 bottle 构建环境决定。开发机链接运行成功不等于已经生成兼容
macOS 12 的便携发行包；发布前应另外检查 `otool -L`、deployment target、签名和目标机器运行结果。
...
移植到 Linux 时，需要提供与目标架构匹配的 SDL3/SDL3_ttf 共享库，并相应调整发布脚本。字体候选
路径已经包含常见 Windows、macOS 与 Linux 位置，但应在目标发行版上运行 `sdl` 字体测试进行确认。
