...
一段长短不一的对话，演示 `LazyList` 变高虚拟列表：每条气泡按预估高度定位，只物化视口附近的十几条，上翻
几百条历史消息仍只付出一屏成本。气泡左右分列（我方右、蓝底白字；对方左、白底深字），正文按估算行数用
`maxLines` 封顶以吻合预估高度、绝不超高裁切。输入栏发送后追加消息并把滚动置到底部（`LazyList` 会把超出
偏移钳到底），最新消息始终可见。行偏移是各高度的累加和、可见窗口由二分定位，`heightOf` 只需 O(1)。

### [`activity`](activity/README.md)：文件活动流（富文本）

一条条文件操作事件，演示 `RichText` 内联样式文本：每条活动把行首按动作着色的图标、强调色的操作者、常规色
动词、动作色的文件路径与弱化色的时间拼在一行；长文件路径在卡片宽度内自动换行、图标随文流动。文本段共享
同一字号、以颜色区分（渲染层无粗斜体，故 `RichText` 走多色 + 内联图标这条实现路径）。顶部分段控件按动作
类型筛选（全部/新建/保存/复制/删除/打开），列表由 `filtered()` 派生。是 `Label`（单一样式）的图文混排对应物。
...
一个会议排期界面，演示 `TimePicker` 时间选择：弹出的时/分网格选时，点选实时更新、方向键微调（上下按分步、
左右按小时，跨午夜环绕），与 `DatePicker` 并排组成一行日程表单。开始/结束时间用纯函数 `TimeOfDay` 实时算出时长，
...
一个模拟文件浏览器，演示 `Breadcrumb` 面包屑路径导航：顶部显示 根目录 › … › 当前 的层级路径，点任意一段
跳回该层，末段为当前位置（不可点），路径过长时中间自动折叠为省略号。下方列出当前目录内容，点文件夹进入
（压入路径），与面包屑构成完整的上下导航。模拟目录树、沿路径查找、面包屑分段与导航都是纯函数，便于单测。
