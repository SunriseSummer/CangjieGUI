...
from test_desktop_lifecycle import PASS_MARKER, SCENARIOS

ROOT = Path(__file__).resolve().parent.parent
SDL_ROOT = ROOT.parent / "CangjieSDL"
FIXTURE = ROOT / ".devtools" / "fixtures" / "desktop_lifecycle"
RESULTS = ROOT / "target" / "cross-platform"
...
def executable_path():
    ...
    path = FIXTURE / "target" / "release" / "bin" / name
    if not path.is_file():
    ...
    try:
        ...
        if system == "windows":
            native = run_stage([
                ...
                str(ROOT / ".devtools" / "build_windows_uia.ps1"),
