...
WORKFLOW = Path(__file__).resolve().parent.parent / ".github" / "workflows" / \
    "cross-platform-qualification.yml"
...
class CrossPlatformWorkflowTests(unittest.TestCase):
    ...
    def test_hosted_portability_lane_covers_fresh_linux_and_arm_macos(self):
        ...
        for command in (
                "python .devtools/bootstrap_cross_platform.py",
                "python .devtools/build_sdl_runtime.py",
        ...
        for contract_test in (
            ...
            self.assertIn(contract_test, self.portable)
            self.assertIn(contract_test, self.qualify)
    ...
    def test_both_repositories_and_explicit_runtimes_are_staged(self):
        for lane in (self.portable, self.qualify):
            ...
            self.assertIn("python .devtools/stage_sdl_runtime.py", lane)
