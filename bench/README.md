...
python bench/run.py --display --samples 5 --capture-baseline-candidate  # 采集完整、可审阅候选
python bench/run.py --promote-baseline-candidate  # 独立校验并晋升候选，唯一活动基线写入口
python bench/run.py --display --samples 5 --check  # 稳定 AC 环境下独立裁决；电池只观测
...
powershell -File bench/native/windows/build_sdl_gpu_msaa_probe.ps1 -Run # Windows 原生 MSAA/SSAA 隔离 A/B
powershell -File bench/native/windows/build_sdl_gpu_scene_probe.ps1 -Run # 几何/clip/alpha/atlas 代表场景裁决
cd bench/micro && cjpm run --run-args paint-outset  # 10k 零值/大非对称绘制域 A/B
cd bench/micro && cjpm run --run-args retained       # 显式 retained 命中/替换与生命周期隔离采样
cd bench/micro && cjpm run --run-args retained-hit   # 新进程隔离稳定命中，避免 miss/GC 污染
cd bench/micro && cjpm run --run-args retained-miss  # 新进程隔离 revision 变化冷路径
cd bench/micro && cjpm run --run-args layout         # ≥约 100ms 的稳定/重建深层布局口径
cd bench/micro && cjpm run --run-args state          # State 事务合并与 1/256 观察者直接通知
cd bench/micro && cjpm run --run-args derived        # 缓存/重算、外置/remembered/内联派生与手工 A/B
cd bench/micro && cjpm run --run-args binding        # 深层 Binding 与朴素/endomorphism-fused Lens A/B
cd bench/micro && cjpm run --run-args automatic      # 自动脏路径、keyless/显式状态、显示列表 A/B
cd bench/micro && cjpm run --run-args state-scale    # 1k/10k build scope + 稳定/替换 phase State 分相曲线
cd bench/micro && cjpm run --run-args semantics-scale # 1k/10k 提交、稳定帧、动作与空间查询
cd bench/micro && cjpm run --run-args overlay-scale   # 1k/4k keyed 注册、事件与稳定片段采用
cd bench/micro && cjpm run --run-args modifier        # 1/8/128/1024/8192 步左结合/平衡 Modifier A/B
cd bench/micro && cjpm run --run-args reducer-concat  # 8/128/1024 项 Reducer 左结合/平衡 A/B
python bench/run_all.py --display --samples 5   # Python/驱动自测 + 三层套件完整执行
python bench/run_all.py --display --samples 5 --capture-baseline-candidate  # 完整套件并暂存候选
```
...
`baseline.meta.json` 写入 `explicit-reviewed-candidate-v1` 晋升记录。旧 `--save-baseline` 只保留为候选采集兼容
别名，不能直接覆盖活动门禁；`--check` 也不能与采集动作组合。电池采集可带噪声警告保存为 observational
candidate 供趋势分析，但晋升必然拒绝。`baseline.json` 保存活动数值，`baseline.meta.json` 保存 `verdictMode`、
源码、环境、后端、样本稳定性与同进程 A/B 来源；缺少元数据的历史基线不参与裁决。
基线按目标平台和架构隔离，不能跨机器族比较：为兼容已有历史，Windows x64 使用 `baseline.json` /
...
- `native/windows/`：固定到 SDL 3.4.12 ABI 的 Windows 原生压力探针；以交替 ABBA/BAAB、最终 GPU 排空和
...
候选采集保留各用例平均帧成本及完整来源，显式晋升才写入 `bench/baseline.json` / `baseline.meta.json`；
