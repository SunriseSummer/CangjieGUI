...
`verify-macos.sh` 验证框架与默认 Notepad 示例的编译链接。设置 `CANGJIEGUI_RUN_TESTS=1` 时还会运行
...
仓库随软件附带未修改的 HarmonyOS Sans SC，作为跨平台中文字体回退。运行时可通过
`CANGJIEGUI_FONT_PATH=/absolute/path/to/font.ttf` 显式指定应用字体；未指定时依次尝试仓库字体和系统
字体。HarmonyOS Sans SC 的版权归 Huawei Device Co., Ltd. 所有，并按
[HarmonyOS Sans Fonts License Agreement](assets/fonts/HARMONYOS_SANS_LICENSE.txt) 使用和再分发。
